package com.capgemini.investment.aspect;
...
class LoggingAspectTest {
    ...
    void logServiceMethods_whenSucceeds_shouldReturnResult() throws Throwable {
        ...
        when(signature.toShortString()).thenReturn("InvestmentService.createInvestment()");
    ...
    void logServiceMethods_whenReturnsNull_shouldReturnNull() throws Throwable {
        when(pjp.getSignature()).thenReturn(signature);
        when(signature.toShortString()).thenReturn("InvestmentService.approveInvestment()");
    ...
    void logServiceMethods_whenThrowsException_shouldRethrow() throws Throwable {
        when(pjp.getSignature()).thenReturn(signature);
        when(signature.toShortString()).thenReturn("InvestmentService.getInvestment()");
    ...
    void logServiceMethods_whenThrowsCheckedException_shouldRethrow() throws Throwable {
        when(pjp.getSignature()).thenReturn(signature);
        when(signature.toShortString()).thenReturn("InvestmentService.process()");
