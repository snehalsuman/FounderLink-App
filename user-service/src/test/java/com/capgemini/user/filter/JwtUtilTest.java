...
class JwtUtilTest {
    ...
    void extractRoles_shouldReturnCorrectRoles() {
        ...
        assertThat(roles).containsExactly("INVESTOR");
    }

    @Test
    void extractAllClaims_shouldContainExpectedClaims() {
        String token = buildToken(5L, "bob@example.com", "COFOUNDER");

        assertThat(jwtUtil.extractAllClaims(token).getSubject()).isEqualTo("bob@example.com");
