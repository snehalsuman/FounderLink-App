...
class JwtUtilTest {
    ...
    void extractRoles_shouldReturnRolesList() {
        String token = buildToken(1L, "alice@example.com", "FOUNDER");
        assertThat(jwtUtil.extractRoles(token)).containsExactly("FOUNDER");
    }

    @Test
    void extractAllClaims_shouldReturnValidClaims() {
        String token = buildToken(7L, "carol@example.com", "CO_FOUNDER");
        assertThat(jwtUtil.extractAllClaims(token).getSubject()).isEqualTo("carol@example.com");
    }
}
