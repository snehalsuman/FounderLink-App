...
class JwtAuthenticationFilterTest {
    ...
    @Test
    void doFilterInternal_withMultipleRolesHeader_shouldSetAllAuthorities() throws Exception {
        when(request.getHeader("X-User-Id")).thenReturn("5");
        when(request.getHeader("X-User-Roles")).thenReturn("ROLE_FOUNDER,ROLE_INVESTOR");
    ...
    void doFilterInternal_withValidBearerToken_shouldSetAuthentication() throws Exception {
        ...
        when(jwtUtil.extractUserId("valid.jwt.token")).thenReturn(99L);
        when(jwtUtil.extractRoles("valid.jwt.token")).thenReturn(List.of("ROLE_FOUNDER"));
